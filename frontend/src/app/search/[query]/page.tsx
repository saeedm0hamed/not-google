...
export default function DynamicSearchPage({ params }: { params: Promise<{ query: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`https://sae8d-not-google.hf.space/search?q=${rawQuery}&k=100`);
            ...
            {loading ? (
              ...
              <>
                ...
                {currentResults.map((result) => (
                  <div className='mb-8' key={result.rank}>
                    <div className='mb-1'>
                      <a
                ...
                {totalResults === 0 && (
                  <div className='p-8 text-center outset-bevel bg-[#c0c0c0]'>
